...
In this tutorial, you will learn how to write content for tutorials.ekinops.
...
* How to get it ready for review by the Ekinops Docs team
...
summary: Learn how to create, write and publish tutorials on tutorials.ekinops, reaching a wide audience of both beginner and advanced Linux users.
categories: community
tags: guidelines, guide, write, contribute
difficulty: 2
status: published
image: https://foo.png
feedback_url: https://ekinops.com/issues
published: 2018-05-17
author: John Doe <john.doe@ekinops.com>
 ...
 * **difficulty**: the scale spans from 1 to 5. Beginners without previous knowledge of the given topic should be able to follow and understand tutorials from level 1 without any other prerequisite knowledge. As a guide:
     ...
     * **1**: Complete beginner at Ekinops
     * **2**: Ekinops novice - can be trusted to enter commands but experience limited to simple file operations
     * **3**: Experienced user - doesn't need explanations about common topics (networking, sudo, services)
     * **4**: Advanced user - has experience running Ekinops products for many years, but may be unfamiliar with some sysadmin/programming topics
     * **5**: Ekinops sysadmin/developer- very familiar with most aspects of OneOs6 and can be expected to know details about its inner workings
 ...
 * **feedback_url**: where to report bugs and give feedback about this tutorial. Unless you have very specific requirements, this should be: `https://ekinops.com/issues`.
 * **author**: the name and email address between brackets of the author of the tutorial. If you don't intend to maintain this tutorial after publication, please use `Ekinops Team <team@ekinops.com>`
...
First impressions matter and the first page of your tutorial should be welcoming and informative. On tutorials.ekinops, an "Overview" page should be the first step of every tutorial.
...
- How to install OA Tools
- How to install OA SDKs
- How to create a OneOs6 module
```
...
We are going to need a local instance of tutorials.ekinops, which is something we can achieve without any advanced technical knowledge: it takes 5 minutes and a single command handles all the process.
...
Then enter the `tutorials.ekinops` directory and launch the `run` script.

```bash
cd tutorials.ekinops
./run
```
...
The tutorials.ekinops source code contains a `tutorials` directory. Each of its subdirectories matches a tutorial category: `cloud`, `server`, `packaging`, etc. Your tutorial should be included into one of these.
...
If no category is matching your topic, you can propose a new category by opening an issue.
...
Push it, as a branch of the project:

```bash
git push -u origin <tutorial-id>
```

You will then be asked to enter your git credentials.
TODO change URL
Once your `push` has been processed, open the [project page](https://github.com/canonical-websites/tutorials.ubuntu.com). You should see a highlighted box near the top of the page with your newly pushed git branch name (the tutorial id) on it. Click on the "Pull Request" button next to it.
...
* Read other tutorials and help us improving them by opening new issues
